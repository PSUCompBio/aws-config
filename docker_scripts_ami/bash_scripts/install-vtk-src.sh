...
#            -DPYTHON_INCLUDE_DIR=$(python2 -c "from distutils.sysconfig import get_python_inc; print(get_python_inc())") \
#            -DPYTHON_LIBRARY=/usr/lib64/libpython.2.7.so
cmake3 .. -DBUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=Release -DVTK_OPENGL_HAS_OSMESA=ON  -DVTK_USE_X=OFF -DVTK_WRAP_PYTHON=OFF -DBUILD_EXAMPLES=OFF -DBUILD_TESTING=OFF -DVTK_RENDERING_BACKEND=OpenGL
